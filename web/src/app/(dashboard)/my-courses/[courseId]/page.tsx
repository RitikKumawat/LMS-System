...
export default function CourseDetailPage() {
  ...
  const { data: courseProgress } = useQuery(GetCourseProgressDocument, {
    variables: { courseId: courseId as string },
    skip: !courseId,
    fetchPolicy: "network-only",
  });
  ...
  const [createCourseOrder, { loading: creatingOrder }] = useMutation(
    CreateCourseOrderDocument,
  );
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      notifications.show({
        ...
        message: message || "Unable to download certificate right now.",
        color: "red",
      });
    }
  };
  ...
  const handleDownloadCertificate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/certificates/download/${courseId}`,
        {
          method: "GET",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          "Certificate is not available yet. Please try again shortly.",
        );
      }
      ...
      const fileNameMatch = response.headers
        ...
        ?.match(/filename="?([^\"]+)"?/i);
      const fileName = fileNameMatch?.[1] ?? `certificate-${courseId}.pdf`;

      const reader = response.body?.getReader();
      if (!reader) {
        throw new Error("Unable to read certificate stream.");
      }
      ...
      while (true) {
        ...
        if (value) {
          // Convert to proper ArrayBuffer-backed Uint8Array
          const safeChunk = new Uint8Array(value.buffer.slice(0));
          ...
          if (contentLength > 0) {
            setDownloadPercentage(
              Math.min(100, Math.round((receivedLength / contentLength) * 100)),
            );
          }
        }
      }
      ...
      setTimeout(() => setDownloadPercentage(0), 600);
    }
  };
      ...
      <Grid gutter="xl">
        <Grid.Col span={{ base: 12, md: 8 }}>
          <Stack gap="lg">
            ...
            <CourseModulesAccordion courseId={courseId!} isEnrolled={isEnrolled} />
          </Stack>
        </Grid.Col>
              ...
              <Stack gap="md">
                ...
                {isEnrolled &&
                  ...
                  courseProgress?.getCourseProgress?.percentage > 0 && (
                    <GlassProgressBar
                      value={
                        courseProgress?.getCourseProgress.percentage as number
                      }
                    />
                  )}
