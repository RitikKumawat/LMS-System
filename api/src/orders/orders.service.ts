...
export class OrdersService {
  ...
  async create(req: Request, course_id: string): Promise<CourseOrderResponse> {
  ...
  async updateByRazorpayOrderId(razorpayOrderId: string, status: OrderStatus) {
  ...
  private async handlePaymentCaptured(payload: any) {
    ...
    const existingPayment =
      await this.paymentService.findByRazorpayPaymentId(razorpayPaymentId);

    if (existingPayment) return;

    const order = await this.findByRazorpayOrderId(razorpayOrderId);
    ...
    await this.lessonProgressService.unlockInitialLessons(
